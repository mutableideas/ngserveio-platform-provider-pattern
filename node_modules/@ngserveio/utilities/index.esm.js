 ...
 */
function isToday(dt) {
    const dt1 = DateTime.fromJSDate(dt).startOf('day');
...
function startOf(unit, dt) {
    const value = dt ? DateTime.fromMillis(dt) : DateTime.now();
    return value.startOf(unit).toMillis();
}
function endOf(unit, dt) {
    const momentValue = dt ? DateTime.fromMillis(dt) : DateTime.now();
    return momentValue.endOf(unit).toMillis();
}
function formatShortMonthDayYear(dt) {
    return DateTime.fromMillis(dt).toFormat('DD');
}
...
export { addDays, allEmpty, anyEmpty, chunkAsync, chunkMap, convertToQueryStringParams, covertPartialToDotPathKeys, dayOfWeek, defaultIfEmpty, distinct, endOf, endOfMonth, endOfWeek, formatShortMonthDayYear, formatString, getFileExtension, getNestedValue, getRandomNumber, groupBy, groupDictionary, hasKeys, isBoolean, isEmail, isEmpty, isEmptyOrWhiteSpace, isFunction, isNullOrUndefined, isNumber, isObject, isPrimitive, isString, isToday, isUrl, months, notEmpty, omit, patchValues, pick, removeProperty, sortBy, startOf, startOfMonth, startOfWeek, startOfYear, toDictionary, today, wait };
